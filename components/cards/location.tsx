...
export function LocationCard() {
  ...
  React.useEffect(() => {
    ...
    const globe = createGlobe(canvasRef.current, {
      ...
      glowColor: resolvedTheme === "dark" ? [0.2, 0.2, 0.2] : [0.5, 0.5, 0.5],
      markers: [{ location: [6.5244, 3.3792], size: 0.15 }],
      scale: 1,
      onRender: (state) => {
        state.phi = phi;

        if (state.phi > 5.5) direction = -1;
        else if (state.phi < 4.0) direction = 1;
  ...
  return (
    <motion.div
      ...
      className="card-border relative col-span-4 col-end-[-1] row-span-3 row-start-4 flex flex-col gap-6 overflow-hidden rounded-xl bg-white p-4 dark:bg-neutral-900 md:col-span-2 md:col-start-6 md:row-span-2 md:row-start-2 md:h-40">
      <div className="z-10 flex items-center gap-2">
        <Globe className="size-[18px]" />
        <h2 className="text-sm font-light">Lagos</h2>
      </div>
