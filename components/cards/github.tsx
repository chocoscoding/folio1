...
export function GithubStatsCard({
  ...
  return (
    <MotionLink
      ...
      className="card-border relative col-start-5 col-end-[-1] row-start-1 row-span-2 overflow-hidden rounded-xl bg-white p-4 dark:bg-neutral-900 md:col-span-3 md:col-start-3 md:row-span-2 md:row-start-1">
...
function ContributionsGraph({ contributions }: { contributions: Awaited<ReturnType<typeof getGithubContributions>> }) {
  return (
    <ul className="absolute inset-y-0 right-0 z-0 flex max-h-full gap-1 overflow-hidden opacity-50">
      ...
      {contributions.latestContributions.map((week, weekIndex) => {
        ...
        return (
          <li key={`contributions-week-${weekIndex}`} className="flex aspect-[1/8] size-full flex-col gap-1">
            {week.contributionDays.map((day) => {
              return (
                <div
                  key={`contribution-week-${weekIndex}-day-${day.date}`}
                  className={cn("flex aspect-square rounded-[3px]", contributionsColorMap[day.color])}
                />
              );
            })}
          </li>
